...
function showSuggestions() {
    ...
    if (matches.length > 0) {
        matches.forEach(match => {
            ...
            item.innerText = match.replace(".html", ""); // 去掉 .html 显示
...
document.body.style.cursor = "url('./asset/media/New Folder/mouse.jpg') 0 0, auto";
///////////////////////////////////////////////////////////////
...
function showSuggestions() {
    ...
    if (matches.length > 0) {
        matches.forEach(match => {
            ...
            item.classList.add("list-group-item", "list-group-item-action");
            item.innerText = match.title; // 显示标题，不显示文件名
            suggestionsDiv.appendChild(item);
        });
    } else {
        let noMatch = document.createElement("div");
...
function setLocalStorage(name, value, days) {
    ...
    if (days) {
        ...
        data.expiry = expireDate.getTime(); // 过期时间存储为时间戳
    }
...
function getLocalStorage(name) {
    ...
    if (!item) return null; // 如果不存在，返回 null

    let data = JSON.parse(item);

    // 检查是否设置了过期时间，并且是否已过期
    if (data.expiry && new Date().getTime() > data.expiry) {
        localStorage.removeItem(name); // 过期了，删除
        return null;
    }

    return data.value; // 返回实际存储的值
}